  ...
  describe '.do' do
    ...
    context 'scopes' do
      context 'scope all' do
        it 'set scope all' do
          params[:scopes] += ['all']
          expect(ThinkingSphinx).to receive(:search).with(params[:q], page: 1)
          Search.do(params)
        end
      ...
      it 'classify scope' do
        ...
        expect(ThinkingSphinx).to receive(:search).with(params[:q], classes: classes, page: 1)
        Search.do(params)
      end
      ...
      context 'pages' do
        ...
        it 'set page' do
          ...
          expect(ThinkingSphinx).to receive(:search).with(params[:q], page: params[:page])
          Search.do(params)
        end
      end
    end
